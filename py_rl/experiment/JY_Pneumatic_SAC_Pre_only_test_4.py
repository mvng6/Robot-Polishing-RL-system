# 250826 강화학습 에이전트가 데이터 처리 시간 모니터링 기능 추가하기
...
CONFIG = {
    ...
    "R_SLEW_PER_40MS": 0.048,    # |Δresidual| limit per 40ms tick (25Hz 기준)
    # Scheduling - 25Hz로 변경 완료
    "TICK_SEC": 0.04,            # 25Hz (40ms) - 정확한 타이밍
    "TICK_TOL": 0.01,            # 10ms 허용 오차 - 25Hz에 맞는 설정
    # Training
    "BATCH_SIZE": 64,
    "REPLAY_WARMUP": 100,
    # Networking
    ...
    # Episode - 25Hz에 맞게 조정
    "EPISODES": 10,
    "MAX_EPISODE_STEPS": 750,    # 30초 (750 * 0.04초)
    # Safety / Reward shaping
    "MAX_FORCE_ERR": 15.0,
    "MAX_PRESS_DELTA": 0.05,
    # Logging - 25Hz에 맞게 조정
    "LOG_EVERY_CTRL": 25,        # 1초마다 로깅 (25 * 40ms)
    "SAVE_THRESH_FREQ": 5,
    "SAVE_THRESH_PCT": 80,
    # Model saving  
...
class ResidualRLCommunicator:
    def __init__(self, host, port, recv_timeout):
        ...
        self.missed_packets = 0  # 누락된 패킷 수 (오류 방지용)
        self.late_packets = 0  # 지연된 패킷 수 (오류 방지용)
...
class PneumaticPolishingEnvironment:
    ...
    def _log_send_data(self, episode, step, residual_mpa, timing_accurate, episode_done, sander_active, state):
        """전송 데이터를 CSV 파일과 메모리에 로깅 - 50Hz 기준, 순서 변경 및 이름 명확화"""
        ...
        # 메모리에 에피소드별 데이터 저장 (그래프 생성용) - 50Hz 기준, 순서 변경 및 이름 명확화
    ...
    def should_send_now(self):
        """정확한 25Hz 타이밍 제어 - 허용오차 10ms"""
        now = time.perf_counter()
        if self.last_tick is None:
            self.last_tick = now
            return True, True
        dt = now - self.last_tick
        # 25Hz = 40ms 간격, 정확한 타이밍
        if dt >= self.cfg["TICK_SEC"]:
            # 허용오차 10ms로 설정 - 정확하게 40ms마다
    ...
    def run_training(self, episodes=None):
        ...
        self._log("INFO", "🚀 CLEANED VERSION: TRUE Residual RL - 10 episodes, 30 seconds each")
        self._log("INFO", "📡 RL sends residual at 25Hz, Robot sums PI(1kHz)+Residual. CSV logging also at 25Hz.")
        self._log("INFO", "⏱️  Each episode: 750 steps (30 seconds) - 정확한 25Hz 타이밍")
        self._log("INFO", "🎯 Episode completion: Only when max steps (750) reached")
        ...
        for ep in range(episodes):
            ...
            while True:
                ...
                # 🎯 25Hz 기준으로만 스텝 증가
                self.episode_step += 1
                # (1) store prev transition & learn on 25Hz boundary
                ...
                if not ok:
                    ...
                    #     print(f"📡 25Hz Send: residual={rl_residual:.3f}MPa, episode_done={episode_done}, timing_accurate={timing_accurate}")
                # 🎯 에피소드 종료 시 즉시 break
                if episode_done:
                    self._log("INFO", f"🎯 Episode {ep+1} completed at step {self.episode_step:,}")
                    # 1. episode_done=True 여러 번 전송 (C++ 처리 보장)
                    self._log("INFO", f"🔧 Sending episode_done=True multiple times (expecting target_force to change from {self.previous_target_force:.1f}N)")
                    for i in range(5):
