...
# Utils: seed
# =========================
def set_seed(seed=42):          # 42는 딥러닝에서 표준적으로 사용되는 시드값
...
class ResidualSACAgent:
    ...
    def save_model(self, path):
        ...
        print(f"💾 Saved: {path}")
# =========================
# TCP Communicator (residual only) - 로봇 PC와 통신
# =========================
class ResidualRLCommunicator:
    def __init__(self, host, port, recv_timeout): 
        self.host, self.port = host, port            # 서버 주소와 포트 저장
        self.recv_timeout = recv_timeout             # 수신 타임아웃 설정
        self.socket = None                           # 서버 소켓
        self.conn = None                             # 클라이언트 연결 소켓
        ...
        self.expected_packet_interval = 0.001
