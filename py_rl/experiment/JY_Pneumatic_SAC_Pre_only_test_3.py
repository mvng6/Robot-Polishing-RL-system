# episode_done 코드 수정 전까지의 백업 코드 파일
...
# =========================
# CONFIG - CLEANED VERSION
# =========================
CONFIG = {
    # Neural Network
    "STATE_DIM": 6,              # 6개 상태 변수
    "ACTION_DIM": 1,
    "HIDDEN": 256,
    "LR": 3e-4,
    "GAMMA": 0.99,
    "TAU": 0.005,
    "AUTO_ENTROPY": True,
    # Residual limits (MPa) - 최대 학습 효과를 위한 확장
    "R_MIN": -0.2,                # -200 kPa (시스템 100% 활용)
    "R_MAX":  0.2,                # +200 kPa (시스템 100% 활용)
    "R_SLEW_PER_100MS": 0.12,    # |Δresidual| limit per 100ms tick (10Hz 기준)
    # Scheduling - 10Hz로 변경 완료
    "TICK_SEC": 0.100,           # 10Hz (100ms) - 정확한 타이밍
    "TICK_TOL": 0.005,           # 5ms 허용 오차 - 현실적인 설정
    # Training
    "BATCH_SIZE": 64,
    "REPLAY_WARMUP": 100,
    # Networking
    ...
    # Episode - 10Hz에 맞게 조정
    "EPISODES": 10,
    "MAX_EPISODE_STEPS": 500,  # 50초 (500 * 0.1초)
...
class ResidualSACAgent:
    ...
    def save_model(self, path):
        ...
        print(f"💾 Saved: {path}")
# =========================
# TCP Communicator (residual only) - CLEANED
# =========================
class ResidualRLCommunicator:
    ...
    def connect(self):
        try:
            ...
            # 소켓에 timeout 설정 (Ctrl+C 시그널 처리 가능)
            self.socket.settimeout(1.0)  # 1초마다 timeout으로 시그널 체크
    ...
    def print_communication_stats(self):
        ...
        if 'avg_packet_interval_ms' in stats:
            ...
            self._log("INFO", f"📊 Avg Packet Interval: {stats['avg_packet_interval_ms']:.3f}ms (Target: 1.000ms)")
            self._log("INFO", f"📊 Packet Interval Std: {stats['packet_interval_std_ms']:.3f}ms")
            self._log("INFO", f"📊 Missed Packets: {stats['missed_packets']}")
            self._log("INFO", f"📊 Late Packets: {stats['late_packets']}")
            self._log("INFO", f"📊 Packet Loss Rate: {stats['packet_loss_rate_percent']:.2f}%")
            # 품질 평가
            if stats['packet_loss_rate_percent'] < 0.1:
                ...
                quality = "🔴 POOR"
            self._log("INFO", f"📊 Overall Quality: {quality}")
...
class PneumaticPolishingEnvironment:
    ...
    def _log_send_data(self, episode, step, residual_mpa, timing_accurate, episode_done, sander_active, state):
        """전송 데이터를 CSV 파일과 메모리에 로깅 - 10Hz 기준, 순서 변경 및 이름 명확화"""
        if not self.cfg["LOG_SEND_DATA"] or not self.send_data_logger:
            return
        # 현재 시간 기록
        current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]
        # 데이터 행 구성 - 순서 변경 및 이름 명확화
        data_row = [
        ...
        # 메모리에 에피소드별 데이터 저장 (그래프 생성용) - 10Hz 기준, 순서 변경 및 이름 명확화
    ...
    def run_training(self, episodes=None):
        ...
        self._log("INFO", "🚀 CLEANED VERSION: TRUE Residual RL - 10 episodes, 50 seconds each")
        self._log("INFO", "📡 RL sends residual at 10Hz, Robot sums PI(1kHz)+Residual. CSV logging also at 10Hz.")
        self._log("INFO", "⏱️  Each episode: 500 steps (50 seconds) - 정확한 10Hz 타이밍")
        self._log("INFO", "🎯 Episode completion: Only when max steps (500) reached")
        # 🎯 RL Flag가 True가 될 때까지 대기 (로봇이 Control Step 2에 도달할 때까지)
        self._log("INFO", "\n🔄 Waiting for Robot to reach Control Step 2 (RL activation)...")
        self._log("INFO", "📋 Robot Control Sequence:")
        self._log("INFO", "   Step 0: Robot descending to mold (position control)")
        self._log("INFO", "   Step 1: Contact force stabilization (5 seconds)")
        self._log("INFO", "   Step 2: Sander ON + PI control + RL activation")
        wait_start_time = time.perf_counter()
        while True:
            state, sander_active = self.comm.get_latest_state()
            if sander_active:  # sander_active = True
                wait_duration = time.perf_counter() - wait_start_time
                self._log("INFO", f"🎯 RL Activated! (waited {wait_duration:.1f}s)")
                self._log("INFO", "🚀 Starting RL episodes...")
                break
            # 대기 중 상태 표시
            if state is not None:
            ...
            if time.perf_counter() - wait_start_time > 300:
                ...
                self._log("INFO", "🔍 Check robot control settings and physical contact")
                return
        # 🎯 데이터 로깅 시스템 초기화
        ...
        for ep in range(episodes):
            ...
            self.reset_episode()
            # 새 에피소드 시작 신호 전송
            self._log("INFO", f"📡 Starting episode {ep+1} - sending episode_done=False")
            ...
            while True:
                ...
                self.previous_target_force = state[1]  # target_force
                # 10Hz 타이밍 체크 (한 번만 호출)
                send_now, timing_accurate = self.should_send_now()
                if not send_now:
                    continue
                # 🎯 10Hz 기준으로만 스텝 증가 (1000Hz가 아닌)
                self.episode_step += 1
                # (1) store prev transition & learn on 10Hz boundary
                ...
                if self.episode_step >= self.max_episode_steps:
                    ...
                    self._log("INFO", f"🎯 Episode {ep+1} ending at step {self.episode_step} - sending episode_done=True")
                else:
                    episode_done = False
                    if sander_active:
                        # 🎯 10Hz로만 select_action 호출 (1000Hz가 아닌)
                ...
                if not ok:
                    ...
                    #     print(f"📡 10Hz Send: residual={rl_residual:.3f}MPa, episode_done={episode_done}, timing_accurate={timing_accurate}")
                # 🎯 에피소드 종료 시 즉시 break
                if episode_done:
                    self._log("INFO", f"🎯 Episode {ep+1} completed at step {self.episode_step:,}")
                    # 1. episode_done=True 전송 (한 번만)
                    ...
                    while not episode_end_confirmed and (time.perf_counter() - wait_start) < 5.0:
                        ...
                        if state is not None:
                            ...
                            if abs(current_target - self.previous_target_force) > 1.0:
                                ...
                                self._log("INFO", f"✅ Episode end confirmed! New target: {current_target:.1f}N")
                                break
                        time.sleep(0.01)
                    if not episode_end_confirmed:
                        self._log("WARNING", "⚠️ Warning: Episode end not confirmed by Robot PC")
                    break
                ...
                if (self.last_log_time is None or 
                    ...
                    self._log("INFO", f"[Ep {ep+1}/10] Step {self.episode_step} | {mode} | "
                          f"F {state[0]:.1f}/{state[1]:.1f}N | "
                          f"PI {state[5]:.3f}MPa | RL {rl_residual:.3f}MPa | "
                          f"{'EXACT' if timing_accurate else 'DELAY'} | "
...
if __name__ == "__main__":
    ...
    print("🧪 CLEANED VERSION: JY_Pneumatic_SAC_Pre_only_test_3.py")
    print("=" * 60)
    set_seed(42)
