...
# =========================
# CONFIG - TEST VERSION - 시스텝 파라미터들
# =========================
CONFIG = {
    "STATE_DIM": 6,              # 상태 변수: current force, target force, force error, force error dot, force error int, pi output
    "ACTION_DIM": 1,             # 행동 변수: residual pressure
    "HIDDEN": 256,               # 은닉층 크기
    "LR": 3e-4,                  # 학습률
    "GAMMA": 0.99,               # 할인율
    "TAU": 0.005,                # 모델 업데이트 계수
    "AUTO_ENTROPY": True,        # 자동 엔트로피 조정

    # Residual limits (MPa)
    "R_MIN": -0.1,               # 최소 잔류 압력
    "R_MAX":  0.1,               # 최대 잔류 압력
    "R_SLEW_PER_20MS": 0.02,     # |Δresidual| 20ms당 최대 압력 변화량 (안전 제한)

    # Scheduling - 타이밍 제어
    "TICK_SEC": 0.020,            # 50Hz (20ms 간격)으로 RL 제어
    "TICK_TOL": 0.003,            # 20ms 간격 허용 오차

    # Training - TEST VERSION
    "BATCH_SIZE": 64,             # 학습 배치 크기
    "REPLAY_WARMUP": 100,         # 학습 시작 전 필요한 최소 경험

    # Networking - TCP 통신 설정
    "HOST": "0.0.0.0",            
    "PORT": 8888,
...
class ResidualSACAgent:
    ...
    def store_transition(self, state, action, reward, next_state, done):
        ...
        # 저장된 데이터 개수 추적 (디버깅용)
        if len(self.replay) % 10000 == 0:  # 1만개마다 로깅
