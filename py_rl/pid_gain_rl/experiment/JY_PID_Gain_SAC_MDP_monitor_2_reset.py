...
class RLRealtimeMonitor:
    ...
    def _run(self):
        ...
        FuncAnimation(fig, on_timer, interval=interval_ms, blit=False)
        try:
            plt.show()
        except Exception:
            pass
...
class Constants:
    ...
    DEFAULT_EPISODE_SECONDS = (
        40.0  # 에피소드 길이 증가: relaxation 관찰을 위해
    )
...
def create_initial_state(
    ...
    performance_trend = avg_recent_performance = pid_variance = (
        episode_count
    ) = 0.0
...
class PIDGainOptimizationEnvironment:
    ...
    def __init__(self, cfg=None):
        ...
        self.relax_slope_thr = (
            -2.5
        )  # N/s (힘 하강 기울기 기준: 1초에 2.5N 이상 감소)
    ...
    def _relax_update_and_check(self, now_ts: float) -> bool:
        ...
        # ---- Guard ----
        if (
            self._relax_pid_on_ts is None
            or (now_ts - self._relax_pid_on_ts) < self.relax_guard_s
        ):
            return False
    ...
    def calculate_episode_reward(
        ...
        # 성공 보너스: 연속형(0~0.15)
        success_gain = (
            0.15 * (band_ratio**1.2) * float(np.exp(-overshoot_pct / 0.03))
        )
    ...
    def run_pid_optimization_training(self, episodes=None):
        ...
        for ep in range(episodes):
            ...
            if ep > 0 and self.previous_pid_gains is not None:
                ...
                initial_state = create_initial_state(
                    [], self.cfg["TARGET_FORCE"], base_pid, []
                )
            ...
            if actual_state is not None:
                ...
                actual_initial_state = create_initial_state(
                    [actual_state[0]], self.cfg["TARGET_FORCE"]
                )
                self._log(
            ...
            while (time.perf_counter() - start_time) < self.cfg[
                ...
                if self._relax_update_and_check(now_ts):
                    ...
                    self._log(
                        "WARNING",
                        f"RELAX detected: high-pressure+decreasing OR low-pressure+stuck ({self.relax_window_s:.1f}s window)",
                    )
                    break
            ...
            if self.relax_detected:
                self._log(
                    "WARNING",
                    "🗑️ RELAX episode: skip reward/replay/logs; reset and continue.",
                )
                print(
                    f"🗑️ [RELAX] 에피소드 {ep+1} 폐기: 학습/기록 생략, 로봇 리셋 후 다음 에피소드"
                )
                ...
                if ep < episodes - 1:
                    base_pid_for_next = (
                        self.previous_pid_gains
                        if self.previous_pid_gains is not None
                    ...
                    self._log(
                        "INFO",
                        f"🎯 [RELAX→next] PID: Kp={next_pid_gains[0]:.2f}, Ki={next_pid_gains[1]:.2f}, Kd={next_pid_gains[2]:.2f}",
                    )
                    print(
                ...
                print(
                    f"📤 [RELAX] 다음 에피소드 PID 전송 완료 (episode_done=True)"
                )
...
class ControlPerformanceLogger:
    ...
    def _calculate_learning_progress(self):
        ...
        success_rates = [
            ...
            if ep["success_rate"] is not None
        ]
        ...
        earlier_success = (
            np.mean(success_rates[:-3])
            if len(success_rates) > 3
            else success_rates[0]
        )
    ...
    def _calculate_peak_count(self):
        ...
        try:
            ...
            peaks, _ = find_peaks(
                self.force_data, height=np.mean(self.force_data)
            )
            return len(peaks)
        except ImportError:
            ...
            for i in range(1, len(force_array) - 1):
                if (
                    force_array[i] > force_array[i - 1]
                    and force_array[i] > force_array[i + 1]
                    and force_array[i] > mean_force
                ):
                    peaks += 1
            return peaks
