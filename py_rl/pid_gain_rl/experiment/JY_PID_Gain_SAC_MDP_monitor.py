...
# ======================================================================
# RLRealtimeMonitor (Single window)
# Top: current_force / desired_force (episode-scoped, 10 Hz)
# Bottom: episode_reward (update once per episode)
# - Separate process with TkAgg (no file saving)
# - Episode-bounded force buffers (reset at episode start)
# - Queue(maxsize) + put_nowait(): drop-old, keep-latest
# ======================================================================
...
class RLRealtimeMonitor:
    def __init__(self, title="PID Gain RL Monitor", hz=10, rolling_window=60.0):
        ...
        self.rolling_window = float(rolling_window)  # 롤링 윈도우 (초)
    ...
    def post_force(self, t_sec: float, current_f: float, desired_f: float):
        """스텝 루프에서 10 Hz로 호출 (수신/제어는 1 kHz 유지)"""
    ...
    def _run(self):
        import matplotlib
        # Prefer TkAgg; if headless, fall back to Agg and just drain queue
        tk_ok = True
        try:
        ...
        # Rolling window buffers (continuous monitoring)
        tbuf, cbuf, dbuf = [], [], []
        ep_idx, ep_rew = [], []
        ...
        if not tk_ok:
            ...
            while True:
                ...
                if tp == "quit":
                    break
                # reset_force/force/reward 메시지는 무시(창이 없으므로)
            return

        # ---- Figure layout (single window: top Force, bottom Reward) ----
        fig = plt.figure(figsize=(9, 7.2))
        fig.suptitle(self.title)
        gs = gridspec.GridSpec(2, 1, height_ratios=[2.0, 1.0], hspace=0.35)

        # Top: Force (rolling window)
        ...
        # Bottom: Episode Reward (append on episode end)
...
class Constants:
    ...
    DEFAULT_EPISODE_SECONDS = 10.0    # 에피소드 길이 (초) - PID gain 최적화용
    ...
    # =========================
    # 연속형 지수 스코어용 타우 값들
    TAU_RMSE = 3.0                    # RMSE 지수 감쇠 상수
    TAU_SETTLE = 5.0                  # 정착시간 지수 감쇠 상수
    TAU_VAR = 0.1                     # 분산 지수 감쇠 상수
    TAU_U = 1.0                       # 제어 노력 지수 감쇠 상수
    TAU_DU = 1.0                      # 제어 변화율 지수 감쇠 상수
    
    # Potential-based shaping
    POTENTIAL_GAMMA = 0.99            # Potential shaping gamma
    SHAPING_WARMUP_S = 0.5            # Shaping warmup 시간 (초)
    REWARD_WEIGHT_PROGRESS = 0.05     # Progress shaping 가중치
    ...
    # 제어 성능 임계값
    BAND_TOLERANCE_N = 1.5            # 밴드 허용 오차 (N) - ±1.5N
...
class PIDGainOptimizationEnvironment:
    ...
    def calculate_episode_reward(self, force_data, pi_output_data, target_force=-45.0, episode_len_s=10.0):
        """
    ...
    def run_pid_optimization_training(self, episodes=None):
        ...
        for ep in range(episodes):
            ...
            print(f"⏳ sander_active 상승 에지 대기 중...")
            ...
            # ⭐ sander_active 상승 에지부터 10초 데이터 수집
